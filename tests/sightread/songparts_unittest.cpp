...
BOOST_AUTO_TEST_CASE(drum_notes_of_the_same_colour_and_position_are_merged)
{
    std::vector<SightRead::Note> notes {
        make_drum_note(0), make_drum_note(0),
        make_drum_note(0, 0, SightRead::DRUM_YELLOW)};
    SightRead::NoteTrack track {notes, SightRead::TrackType::Drums,
                                std::make_shared<SightRead::SongGlobalData>()};

    BOOST_CHECK_EQUAL(track.notes().size(), 2U);
}

BOOST_AUTO_TEST_CASE(overlapping_notes_of_the_same_colour_are_normalised)
{
    std::vector<SightRead::Note> notes {make_note(0, 768), make_note(192, 0)};
    SightRead::NoteTrack track {notes, SightRead::TrackType::FiveFret,
                                std::make_shared<SightRead::SongGlobalData>()};
    std::vector<SightRead::Note> required_notes {make_note(0, 192),
                                                 make_note(192, 576)};
...
BOOST_AUTO_TEST_SUITE(sp_phrases)
...
BOOST_AUTO_TEST_CASE(base_score_is_correct_for_songs_with_sustains)
{
    ...
    SightRead::NoteTrack track_one {
        notes_one, SightRead::TrackType::FiveFret,
        std::make_shared<SightRead::SongGlobalData>()};
    SightRead::NoteTrack track_two {
        notes_two, SightRead::TrackType::FiveFret,
        std::make_shared<SightRead::SongGlobalData>()};
    SightRead::NoteTrack track_three {
        notes_three, SightRead::TrackType::FiveFret,
        std::make_shared<SightRead::SongGlobalData>()};
...
BOOST_AUTO_TEST_CASE(correct_with_cymbals_on_pro_drums)
{
    std::vector<SightRead::Note> notes {
        make_drum_note(0),
    ...
    BOOST_CHECK_EQUAL(track.base_score(), 115);
}

BOOST_AUTO_TEST_CASE(correct_with_charted_cymbals_for_non_pro_drums)
{
    std::vector<SightRead::Note> notes {
        make_drum_note(0),
        make_drum_note(0, 0, SightRead::DRUM_YELLOW, SightRead::FLAGS_CYMBAL)};
...
BOOST_AUTO_TEST_CASE(no_snapping)
{
    ...
    const SightRead::NoteTrack track {notes, SightRead::TrackType::FiveFret,
                                      make_resolution(480)};
...
BOOST_AUTO_TEST_SUITE(disco_flip_flags)

BOOST_AUTO_TEST_CASE(red_toms_are_flagged)
{
...
BOOST_AUTO_TEST_CASE(yellows_are_flagged)
{
...
BOOST_AUTO_TEST_CASE(notes_outside_of_disco_flips_are_not_flagged)
{
...
BOOST_AUTO_TEST_CASE(clears_previously_present_disco_flags)
{
    SightRead::NoteTrack track {{make_drum_note(0, SightRead::DRUM_RED)},
                                SightRead::TrackType::Drums,
                                std::make_shared<SightRead::SongGlobalData>()};
